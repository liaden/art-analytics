...
class BootstrapFormBuilder < ActionView::Helpers::FormBuilder
  ...
  def calendar_field(method, options = {})
    CalendarInput.new(self, @template).calendar_field(method, options)
  end

  def tag_filter_field(method, options = {})
    TagFilterInput.new(self, @template).tag_filter_field(method, options)
  end

  def toggle_field(method, options = {})
    ToggleCollectionInput.new(self, @template).
      toggle_field(method, default_collection_for(method), options)
  end
